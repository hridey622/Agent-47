...
from .message_manager_views import MessageMetadata, MessageManagerState
from browser_use.agent.prompts import AgentMessagePrompt
from ..agent_views import ActionResult, AgentOutput, AgentStepInfo
...
class MessageManager:
	...
	def _init_messages(self) -> None:
		...
		example_action = {
			...
			'args': {
				'selector': {'strategy': 'aria_label', 'value': 'Example Button'}
			}
		}
		tool_calls = [
			{
				...
				'args': {
					'current_state': {
						...
						'next_goal': 'Click on example button',
					},
	...
	def add_state_message(
		self,
		browser_state_dict: Dict[str, Any],
		result: Optional[List[ActionResult]] = None,
		step_info: Optional[AgentStepInfo] = None,
		use_vision=True,
	) -> None:
		"""Add browser state dictionary and previous results as a human message"""
		state_messages_for_llm = []
			...
			for r in result:
				if r.include_in_memory:
					if r.error:
						...
						msg = HumanMessage(content='Action error (explicitly included): ' + last_line)
						self._add_message_with_tokens(msg)
					elif r.extracted_content:
						msg = HumanMessage(content='Action result (explicitly included): ' + str(r.extracted_content))
						self._add_message_with_tokens(msg)
				# Don't add results not marked include_in_memory here, they go into the main state message below
			# result = None # We still want to pass the full result list below
		
		# --- Format state message for LLM --- 
		# Add previous action results (summary, not added to history above)
		if result:
			result_summary_lines = []
			for r in result:
				if r.error:
					result_summary_lines.append(f"Previous Action Error: {r.error}")
				elif r.extracted_content:
					result_summary_lines.append(f"Previous Action Result: {r.extracted_content}")
				# else: result_summary_lines.append(f"Previous Action Successful.") # Optional: Add success message
			if result_summary_lines:
				 state_messages_for_llm.append(f"Results from Last Step:\n" + "\n".join(result_summary_lines))
		
		# Add current state info from dict
		state_lines = [
			f"Current URL: {browser_state_dict.get('url', 'N/A')}",
			f"Page Title: {browser_state_dict.get('title', 'N/A')}",
			f"DOM Info: {browser_state_dict.get('dom_summary', 'N/A')}"
		]
		state_messages_for_llm.append("Current Browser State:\n" + "\n".join(state_lines))
		
		# Combine into one message content string
		state_message_content = "\n\n".join(state_messages_for_llm)

		# Vision handling
		state_message_input: Union[str, List[Union[str, Dict]]] = state_message_content
		if use_vision and browser_state_dict.get('screenshot_base64'):
			 state_message_input = [
				 {"type": "text", "text": state_message_content},
				 {
					 "type": "image_url",
					 "image_url": {"url": f"data:image/png;base64,{browser_state_dict['screenshot_base64']}"},
				 },
			 ]

		# Add the combined state message to history
		state_human_message = HumanMessage(content=state_message_input)
		self._add_message_with_tokens(state_human_message)
